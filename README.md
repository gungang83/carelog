...
- **녹음 엔진 실험실(Engine Lab)** — 실험실 활성 워크스페이스에서 상담별 엔진 선택: 기본모델 / 다국어(자동 언어감지+번역) / 비교(동시 실행 후 선택). 비-lab은 기본모델 강제
- **직원 초대 / 다중 테넌트** — 기관별 완전 격리, 역할(owner / admin / staff) 기반 권한
- **슈퍼어드민(최고 관리자 패널)** — 전체 기관·직원 통합 관리 + 워크스페이스별 실험실 토글 (프로필 메뉴, super 계정 전용)
- **사용량·크레딧 대시보드** — `/admin/usage`(슈퍼어드민): 화면(메뉴) 사용량(역할 분해·미사용) + AI 크레딧 사용을 누가·얼마나·어떤 기능에서 썼는지 기관·직원·기능별 통계 + 잔액·충전(시뮬레이션). 크레딧은 비차단(전사 안 끊김)
- **일일 사용 리포트** — 매일 아침 08:00(KST) 전일(0~24시) 메뉴·토큰·크레딧 사용을 전체 워크스페이스 단위로 집계해 슈퍼어드민에게 알림+웹푸시. 클릭 시 상세 리포트(`/admin/usage/report/[date]`). Claude 실토큰 캡처, 전일 대비·경고 신호 포함
