...
function ActivityRow({ log }: { log: ActivityLogEntry }) {
    ...
    <>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="shrink-0 text-xs text-slate-400">{formatRelative(log.created_at)}</span>
        </div>
        <div className="flex items-center gap-2">
  ...
  return <div className="flex items-start gap-3 rounded-xl px-3 py-2.5">{inner}</div>;
}
