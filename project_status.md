...
- **마이그레이션**(`20260705000003_update_feed.sql`) 실행 필요. 미실행이어도 피드 조회는 동작(결정 상태만 빈 값), 발행 시 공지는 나가고 상태 기록 실패만 안내.
