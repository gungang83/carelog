...
**최종 업데이트**: 2026-08-14 (세션 67d — EO 크레딧 게이트웨이 발효 spec 031: 단일 지갑 전환·검증 통과. 카드 #1099→#1105) · 2026-08-13 (세션 67c — 다중 이미지 삽입 버그 fix·진행 중 상담 원격 종료·홈 카드 5개·크레딧 소진 정책(b) 구현) · 2026-08-09 (세션 67b — 실시간 통역 PoC spec 030 + 예미안 상담자료 시드 33건(Drive 정리) + 엔진 크레딧 표기) · 2026-07-27 (세션 67a — spec 024 논점 확정·중복 워크스페이스 SQL 정리·EO 재발급 구멍 헤임달 해결) · 2026-07-08 (세션 66i — 상담자료 관리 체계 spec 029: 전역 Library(/admin/assets·분과)+큐레이션(카테고리 그릇·Library 브라우저·픽커 카테고리 뷰)+EO식 기능 권한(permission_overrides)+EO 퇴사 자동 비활성. 마이그레이션 20260708000005 필요) · 2026-07-08 (세션 66h — 이어서 상담+녹음 없이 기록 spec 027 ③④: 카드→보드 프리필·기존 기록 업데이트 저장·음성 .prev 보존. DB 변경 없음) · 2026-07-08 (세션 66g — 설정 IA 개편: EO spec-054 벤치마킹, 4그룹+접이식 섹션. DB 변경 없음) · 2026-07-08 (세션 66e — 상담 세션 안전망 spec 027 v1: RecordingGuard 전역 플로팅 필(경과·무활동)·방치 감시(입력+음성 RMS 2신호)·경고 모달(비프·OS알림)·grace 만료 시 자동 종료및저장(보드 콜백)·전역 beforeunload·탭제목 🔴·Document PiP 항상-위·/settings 상담 안전망(idle10/grace5/voice). 마이그레이션 20260708000004 필요) · 2026-07-08 (세션 66d — 견적 빌더 spec 028 + 동의서(a안)·빈 캔버스: '₩ 견적' 빌더(프리셋 treatment_items→[치료비 견적] 평문 블록), 픽커 빈 캔버스(백지/모눈/줄노트→스테이지), consent 카테고리. 마이그레이션 20260708000003 필요) · 2026-07-08 (세션 66c — 상담 스테이지 spec 026: 자료·기록 이미지를 전체화면으로 열어 그리며 설명(주석도구 재사용) → '기록에 담기'로 스냅샷 삽입. 영상 링크 자산(외부 URL) 등록→기록에 삽입→표시 시 자동 링크화(환자 전달). 마이그레이션 20260708000002 필요) · 2026-07-08 (세션 66b — 상담 이미지 라이브러리+상담 캔버스 spec 025: 기관이 설명 이미지를 미리 등록(/settings 상담 자료) → 상담 에디터 '📚 자료' 픽커로 삽입(카테고리·검색·확대 미리보기·즉석 업로드). 에디터 전체화면 + 이미지 글감싸기/가운데 정렬(왼쪽 2장=나란히). 마이그레이션 20260708000001 필요) · 2026-07-08 (세션 66 — 상담 요약 평문화: AI 요약의 마크다운(**·##) 제거. 프롬프트 5경로 [라벨] 구조 평문 지시 + stripMarkdownMarkers로 기존 기록도 표시·복사 시 소급 정리. DB 변경 없음) · 2026-07-05 (세션 65b — 업데이트 피드 spec 023: 대표 전용 /admin/updates 인박스. 세션 마무리마다 lib/update-feed.ts에 업데이트 내역 append(배포=적재) → 취사선택 → 공지 문구 자동 조합·수정 → 발행/보류. update_feed_decisions(RLS deny-all, service_role만). 마이그레이션 20260705000003 실행 완료 ✅) · 2026-07-05 (세션 65 — EO 공지 구조 대조 → spec 022 확정: EO 원본(announcements 단일 테이블·전역=workspace_id NULL·announcement_reads 읽음테이블·타겟팅·draft/published 상태) 읽기 대조 결과 "전역 announcements 분리" 판단 정합 확인. 스키마 수정 없이 확정 — 마이그레이션 20260705000002 실행 완료 ✅) · 2026-07-05 (세션 64 — 공지·업데이트 티커 spec 022: 홈 헤더 아래 전역 공지가 은은히 흐르는 마퀴 한 줄 + '전체보기'(/announcements). 중앙(슈퍼어드민)이 /admin/announcements에서 발행 → 전 워크스페이스 공통. 전역 announcements 테이블(직원 read RLS / service_role write). 마이그레이션 20260705000002 필요) · 2026-07-05 (세션 63 — 녹음 일시정지/재개: 상담보드·체어 오버레이 녹음 바에 '일시정지'/'이어서 녹음' 버튼. MediaRecorder.pause()/resume()로 스트림 유지·타이머 freeze, 청크 모드는 구간 회전 타이머 함께 정지/복원. DB 변경 없음) · 2026-07-05 (세션 62 — 상담 카드 공용화 + 확인 꼬리표 spec 021: /records 카드 처리 기능을 홈과 완전 동일하게 통일(공용 ConsultationCard — 연결/미연결별 전체복사·음성듣기·편집·삭제·환자연결), 담당이 차트 이관 전 확인할 항목을 다는 '확인 필요' 꼬리표(추가·완료·삭제, 확장형 타입) + /records '확인 필요만' 필터. 마이그레이션 20260705000001 필요) · 2026-06-29 (세션 61 — 멀티워크스페이스 RLS fix: get_my_institution_id() limit1 → my_institution_ids() 멤버십 기반. 여러 기관 소속 사용자 전환 복구(대표가 본원 예미안치과 접근 막혔던 문제 해결). 마이그레이션 20260629000003 필요) · 2026-06-29 (세션 60 — 긴 녹음 자동 서버우회 spec 020 FR-008: '상담 종료'도 3분↑ 녹음은 서버 비동기로 자동 전환→전 워크스페이스(비-lab 포함) 'This page couldn't load' 크래시 방지) · 2026-06-29 (세션 59 — 서버 비동기 전사 spec 020: '상담 종료 및 저장'을 서버 위임으로. 음성만 올리고 즉시 종료→워커가 전사·요약→완료 알림. 탭 닫아도 OK. 마이그레이션 transcription_jobs 필요) · 2026-06-29 (세션 58 — 운영자별 일일 리포트 spec 019: 기관 관리자에게 자기 워크스페이스 리포트 발송(알림함·푸시억제) + /reports/daily/[date] 관리자 페이지) · 2026-06-29 (세션 57 — 일일 서버(인프라) 리포트 spec 018: 일일 리포트에 스토리지·DB·신규생성·증가추이 섹션 + 스토리지 급증 경고. 마이그레이션 get_infra_usage 필요) · 2026-06-29 (세션 56 — 이그레스 절감 spec 017: Supabase 5GB 초과 정지→Pro 복구 후 재발방지. 이미지 업로드 압축(webp)+표시 변환+lazy, off 안전장치) · 2026-06-29 (세션 55 — 점진 청크 전사 + 상담 종료/자동저장 spec 016: 녹음 중 구간 백그라운드 전사로 종료 대기 단축 + 종료 및 저장 자동저장·실패 안전망) · 2026-06-29 (세션 54 — 사용량 필터 고도화 spec 015: 기간 직접지정·사용자별 조회·검색형 드롭다운·리포트 날짜선택) · 2026-06-29 (세션 53 — 일일 사용 리포트 spec 014: KST 0~24시 메뉴·토큰·크레딧 집계, 매일 아침 슈퍼어드민 알림+웹푸시, 실토큰 캡처) · 2026-06-28 (세션 52 — 사용량·크레딧 대시보드 spec 013: EO menu-usage+크레딧 벤치마크, 메뉴추적+AI크레딧 원장·통계) · 2026-06-28 (세션 51b — 슈퍼어드민 기관명 깨짐(mojibake) fix: SSO verifyJwt UTF-8 디코드 + 과거데이터 복구) · 2026-06-28 (세션 51 — 알림함 spec 012: EO 벤치마크 포팅, 종+읽음관리+실시간) · 2026-06-28 (세션 50 — /records 빈화면 수정 + 홈 연결완료 카드 통일) · 2026-06-28 (세션 49 — 상담 기록 열람·검색·필터 + 환자별 간략 보기 spec 011) · 2026-06-28 (세션 48 — 예미안 피드백 1차: 중복등록·이름표기·녹음 안내문) · 2026-06-28 (세션 47 — 긴 상담 청크 분할 전사 모드 spec 010 구현: 분할녹음·구간전사·실패격리·복구) · 2026-06-27 (세션 46 — 전사 모드 3종 추가: 빠른메모·상세요약·용어보정, 실험실 픽커) · 2026-06-27 (세션 45 — 긴 상담 녹음 전사 유실 버그 fix: maxDuration page레벨·비트레이트↓·복구 재전사) · 2026-06-26 (세션 44 — 미연결 기록 체어·참여자 편집 + 요약 제목 브랜딩) · 2026-06-25 (세션 43 — 녹음 엔진 picker UX: 홈 히어로 이동·디자인 정리·상담 카피) · 2026-06-24 (세션 42 — C-07 + 오늘의 치과 미팅 기획 + 녹음 엔진 실험실 v1)
**현재 버전**: main 브랜치

---

## 2026-08-14 세션 67d (feat) — EO 크레딧 게이트웨이 발효 (spec 031 · 카드 #1099→#1105)

EO 코어+위성 **단일 크레딧 지갑** 전환 — 하루 만에 계약(헤임달 v1)→구현(다온)→EP·이관(테오)→발효·검증.

- `lib/eo/credit-gateway.ts`: 계약 §2 클라이언트(deduct/balance, `allow_overage:true`·멱등키+동일키 1회 재시도·타임아웃·never-throw). 시크릿은 마스터 게이트웨이 `CARELOG_GATEWAY_SECRET` 재사용.
- `deductCredit`: `EO_CREDIT_GATEWAY=on`이면 EO 단일 원장 차감 + 로컬 `institution_credits`=표시 캐시 강등(이중기록 0). 실패 시 로컬 폴백(fail-open, `[gateway-degraded]` 마커). `getCreditBalance`도 게이트웨이 우선 → 소진 소프트 차단이 EO 잔액 기준으로 자동 전환.
- **이관(#1105 테오)**: 예미안 -73·강남점 -20 상계 + 파일럿 200씩 지급 → EO 잔액 1,017/750. 검증: 전사 1회 → EO 차감·로컬 캐시 1,015 동기화·로컬 원장 신규 0 ✅.
- 후속: /admin 수동 충전 비활성(EO 일원화) · 실시간 통역 reserve→settle 제안 · DB 변경 없음.

## 2026-08-13 세션 67c (fix+feat) — 다중 이미지 버그·원격 종료·홈 정리·소진 정책

- **다중 이미지 삽입 버그 fix**: `setImage`(선택 교체)가 연속 삽입 시 직전 이미지를 덮음 → 이미지+빈 문단 insertContent로 교체(자료·스냅샷·붙여넣기 3경로). 자료 여러 장 담기 정상화.
- **진행 중 상담 원격 종료**: 홈 '진행 중인 상담' 배너에 '종료·저장' — broadcast finalize 커맨드로 작성 중인 기기의 보드가 자동 '종료 및 저장'(미지정 허용) 실행. 다른 컴퓨터에 방치된 상담을 찾으러 다닐 필요 없음.
- **홈 카드 5개 제한**: 기록·활동 기본 5개 + '전체 N개 보기·검색'(/records) 우선, 인라인 펼치기 보조.
- **크레딧 소진 정책(b, 대표 확정)**: 소진(≤0) 시 프리미엄 엔진(상세·용어보정·다국어·비교·통역)만 시작 잠금(서버 게이트+UI 🔒) — 기본 계열은 마이너스 유예. 임박(≤20 고정 임계)·소진 알림 1일 1회(일일 리포트 cron 편승). 정식 서비스화 시 전면 차단(a) 전환 예정.

## 2026-08-09 세션 67b (feat) — 실시간 통역 PoC (spec 030) + 예미안 상담자료 시드

- **실시간 통역(실험실)**: 녹음 중 보드 '🌐 통역' → 전체화면 스테이지(원문 스트리밍 자막 + 문장 확정 시 번역 — 한영 혼용 대응: OpenAI Realtime 전사(ephemeral 토큰, lab 게이트) + Claude Haiku 문장 번역). '기록에 담고 닫기' → `[실시간 통역 — 일시·N분]` 블록으로 상담 기록 삽입. 헤더에 누적 크레딧(분당 1) 실시간 표시 + 종료 정산 안내. 기존 녹음은 병행(사후 전사 안전망). Soniox는 Phase 2 교체 옵션.
- **엔진별 크레딧 차등 표기**: 엔진 선택 pill마다 소모량 뱃지(기본 2·빠른 1·상세/용어/다국어 3·비교 5·긴 상담 구간1+요약2·통역 분당1). 잔액은 ChairProvider 컨텍스트 일원화(히어로·보드 공용).
- **예미안 상담자료 시드 33건**: 김도은 Drive 폴더 정리 → 동의서 5(원본 변환)·주의사항 6(4종 재조판+원본 2p)·사진 8·영상 14(유튜브 6+Drive 8) + 카테고리 7개 큐레이션. Storage 업로드+SQL 시드(사랑니 hwp는 빈 문서라 제외). Drive 영상은 폴더 링크 공유 필요.

## 2026-07-27 세션 67a (docs) — spec 024 확정 + 중복 워크스페이스 정리 + EO 구멍 봉합

- **spec 024 승격**: A 슈퍼어드민 전용·B soft 보관+빈 기관 hard·C EO 연동 삭제 차단만·D 이관 제외 확정 + E(온보딩 중복 예방) 신설. v1 구현 대기.
- **중복 정리(SQL, 코드 무변경)**: EO 링크가 6/28 재발급된 빈 워크스페이스(예미안치과의원)를 가리키던 것을 정본(예미안치과)으로 재지정 → 빈 중복+시드 잔재+테스트 기관 3건 삭제. 멤버 손실 0.
- **EO 재발급 구멍**: 케어로그 토글 OFF→ON 시 새 UUID 발급 → 헤임달 카드 전달 → **EO측 suspend 방식으로 해결 완료**(main@97dbf2f2). 핸드오프 카드는 이후 PLAYBOOK §4-9 표준 양식 사용(CLAUDE.local.md 규칙화).

## 2026-07-08 세션 66i (feat) — 상담자료 관리 체계 (spec 029 전체: 전역·큐레이션·권한)

대표 지시 "다 한꺼번에" — 1·2·3차 통합 구현.
...
- **③ 녹음 없이 기록만**: 히어로 보조 버튼 — 보드를 idle로 오픈(마이크 미사용). 보드는 이미 idle 완비(녹음 시작 폴백·수동 저장)라 진입점만 추가.
- **④ 이어서 상담**: 공용 카드 '🎙 이어서 상담' → `openBoardWithPrefill`(provider ref 핸드오프) → 보드에 기존 본문+`[이어서 상담 — 일시 · 상담: 이름]` 구분 블록·참여자·처방·체어 프리로드 → 저장은 **기존 기록 업데이트**: 수동/클라 전사=updateChairRecordContent, 서버 위임=enqueueServerTranscription `continueConsultationId`(기존 행 update + 워커가 구분 블록 뒤에 새 요약 append). 이어서 모드 배지·안내줄 분기, 버리기/저장 시 모드 해제.
- **음성 원본 보존**: uploadConsultationAudio가 같은 상담 재녹음 시 기존 음성을 `.prev-<ts>`로 이동 후 새 파일 저장(의료기록 보존 — UI 재생은 최신 세션, 과거분은 스토리지 보관).
- ⑥ 보드 확장은 송정현 보드 UX 논의와 묶어 보류. DB 변경 없음.

## 2026-07-08 세션 66g (feat) — 설정 IA 개편 (EO spec-054 벤치마킹)

설정 항목이 11개로 늘며 "찾기 어렵다"(대표) → EO 확정 IA(spec-054: 성격 기준 그룹 + 접이식 섹션 + 정보성 부제) 이식.

- `components/settings/collapsible-section.tsx`: CollapsibleSection(제목·부제·이모지, 접힘 기본) + SettingsGroupHeader(EO 3층 패턴).
- /settings 재구성 — 4그룹: 🙋 내 설정(알림·내 상담 기록/전 직원) · 🩺 상담 운영(체어·멤버·상담 자료·치료 항목·안전망/owner·admin) · 👥 직원·권한(직원 관리+초대) · 🏥 기관·요금(프로필 owner·요금제·일일 리포트 링크).
- 접힘 기본이라 첫 화면이 한눈에 스캔되는 목록으로. 비관리자는 내 설정+요금만 노출(안내 한 줄).

## 2026-07-08 세션 66f (fix) — '상담 기록 시작' 크래시 핫픽스 (훅 순서 위반)

배포 직후 대표가 두 기기(Win/Mac 크롬)에서 '상담 기록 시작' 클릭 시 "This page couldn't load" 크래시 보고. **로컬 재현 하니스**(임시 /crash-test + Playwright 가짜 마이크)로 원인 확정: 세션 66e에서 넣은 자동종료 등록 훅(useRef+useEffect)이 **보드/오버레이의 조기 return 뒤**에 위치 → 보드 오픈 순간 훅 순서 위반("Rendered more hooks") → 렌더 트리 붕괴.
- 수정: 두 컴포넌트 모두 훅을 조기 return 위로 이동(핸들러는 ref 간접 참조). RecordingGuard 에러 바운더리·AudioContext 방어 추가.
- 재검증: 재현 하니스에서 훅 오류 소멸, 녹음 시작→보드→상시 필 정상, 90초 힙 안정 확인 후 배포.
- 교훈: 상시 마운트 컴포넌트(보드·오버레이)에 훅 추가 시 조기 return 위치 확인 필수.

## 2026-07-08 세션 66e (feat) — 상담 세션 안전망 (spec 027 v1: ①②⑤)

7/8 김도은 회의 기능검토 후속. 핵심 발견: X 닫기 후에도 녹음은 이미 지속(Provider 레벨) — 문제는 표시 부재.
...
- 비범위(다음 차수): ③녹음 없이 상담, ④이어서 상담, ⑥보드 확장. 닫힌 오버레이 세션의 자동 저장(경고만 유지).
- **마이그레이션**(`20260708000004_consult_settings.sql`) 실행 필요(컬럼 1개). 미실행이어도 가드는 기본값(10/5/on)으로 동작, 설정 저장만 실패.
...
- **마이그레이션**(`20260708000003_treatment_items.sql`) ✅ 실행 완료(2026-07-08).
