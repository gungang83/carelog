...
| 2026-06-10 | 다온(Carelog) | 테오(EO) | **카드 237** — 카드236 회신 대기 중 Carelog 코드 재분석본. 블로커를 코드 타입과 1:1로 재확정: **★시크릿 값 `CARELOG_GATEWAY_SECRET` + ★연동 CL `institution_id`** 단 2건이 실질 블로커, 나머지(게이트웨이 호스트·응답 스키마·SSO 클레임)는 확인용. `card-237-eo-deploy-blockers-recheck.md`. | ✅ **EO 연동 라이브(2026-06-10, `ce8b107`)**: 시크릿 등록 + 배포 + cron 미들웨어 fix. sync-master 검증 OK — `0e4e85d6`(예미안) `synced(~30)`. 남은 수동확인: SSO 작성자 귀속 |
