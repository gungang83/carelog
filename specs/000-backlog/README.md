...
| 2026-06-20 | 다온(Carelog) | 헤임달 | **카드 480** — Carelog 대시보드 웜 4~5초 해결 보고(진짜 원인=리전 거리: Supabase 도쿄 ↔ Vercel 함수 미국 iad1 → `vercel.json` regions=`hnd1` 코로케이션) + **EO에도 같은 점검 적용 요청**. 재사용 진단 레시피 `docs/ops-region-colocation.md` 동봉. `card-480-region-colocation-eo.md`. | 발급 완료 / EO 점검 회신 대기 |
