...
| 2026-06-10 | 다온(Carelog) | 테오(EO) | **카드 237** — 카드236 회신 대기 중 Carelog 코드 재분석본. 블로커를 코드 타입과 1:1로 재확정: **★시크릿 값 `CARELOG_GATEWAY_SECRET` + ★연동 CL `institution_id`** 단 2건이 실질 블로커, 나머지(게이트웨이 호스트·응답 스키마·SSO 클레임)는 확인용. `card-237-eo-deploy-blockers-recheck.md`. | ⏳ 전달 / 회신 대기 |
