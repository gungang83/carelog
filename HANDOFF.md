...
| spec 026 | 상담 스테이지(크게 열고 그리기→기록에 담기) + 영상 링크 | `20260708000002_consult_assets_video.sql` | ✅ SQL · ✅ main 배포 |
| spec 028 | 견적 빌더(₩ 견적→[치료비 견적] 블록) + 빈 캔버스 + 동의서 카테고리(a안) | `20260708000003_treatment_items.sql` | ✅ SQL · ✅ main 배포 |
| spec 027 | 상담 세션 안전망 v1(상시 표시·방치 자동저장(미지정)·PiP·안전망 설정) + 핫픽스 2건(훅 순서) | `20260708000004_consult_settings.sql` | ✅ SQL · ✅ main 배포 · 대표 동작 확인 |
| 세션 66g | 설정 IA 개편(EO spec-054 벤치마킹 — 4그룹+접이식 섹션) | 없음 | ✅ main 배포 |
| 세션 66h | spec 027 ③④ — 이어서 상담(기존 기록 업데이트·음성 .prev 보존) + 녹음 없이 기록 | 없음 | ✅ main 배포 |
| spec 029 | 상담자료 관리 체계 — 전역 Library(분과)·큐레이션(카테고리 그릇)·기능 권한·EO 퇴사 자동 비활성 | `20260708000005_asset_mgmt_permissions.sql` | ✅ SQL 실행 완료 · main 배포(마무리 중) |
| spec 030 | 실시간 통역 PoC(실험실)·엔진 크레딧 표기·소진 정책(b) | 없음 | ✅ main 배포 · 실측 대기 |
| spec 031 | EO 크레딧 게이트웨이 수렴 — 단일 지갑(카드 #1099→#1105) | 없음(env: EO_CREDIT_GATEWAY=on) | ✅ 발효·검증 통과 · 헤임달 verified 대기 |
| 카드 #1136 | 이을치과(천주희) institution 생성 `8924a75e-a0fc-495b-811c-465ede43ac98` → 테오·아리아 카드 전달 완료 | 없음(/admin 셀프서브) | ⏳ 테오 EO 연동 연결 대기 |
