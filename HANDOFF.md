...
1. **대표님**: `20260705000003_update_feed.sql` 실행 (spec 023 결정 상태 테이블. 미실행이어도 피드 화면은 뜸 — 발행/보류 기록만 안 됨)
2. 대표님 "마무리" 신호 → spec 023 배포(dev/main 머지 → Vercel)
3. 배포 후 `/admin/updates`에서 백필 4건(이미 7/5 공지로 안내됨)은 '보류' 처리 권장
...
| spec 023 | 업데이트 피드(대표 전용 /admin/updates, 취사선택 발행) | `20260705000003_update_feed.sql` | 코드 완료 · ⚠️ SQL 미실행 · main 미배포 |
