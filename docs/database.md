...
### spec 026 확장 (상담 스테이지·영상 링크)
- `consult_assets.kind`(text default 'image': image|video_link) · `link_url`(text) 추가, `image_url` null 허용(영상 링크 자산).
- migration: `20260708000002_consult_assets_video.sql`

## 치료 항목 사전 (spec 028-estimate-builder)
- `treatment_items` — 견적 빌더 프리셋(기관별). 컬럼: `id` · `institution_id`(→institutions cascade) · `name` · `price`(integer 원, 참고 단가) · `display_order` · `active` · `created_at`.
  - RLS: read=멤버십(모든 직원) / write=owner·admin. 견적 자체는 DB 미저장(본문 [치료비 견적] 블록, 형식 고정 — 후속 데이터화 파싱).
- migration: `20260708000003_treatment_items.sql`
