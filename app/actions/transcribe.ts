...
// 세션 66 — 요약은 전자차트(평문)에 붙는다. 마크다운 금지, 섹션은 [라벨] 구조로 통일.
const PLAINTEXT_RULES = `표기 규칙(중요): 이 기록은 전자차트에 평문 그대로 붙습니다. 굵게(**)·제목(#)·구분선(---) 같은 마크다운 기호를 절대 쓰지 마세요.
섹션 제목은 [진찰 소견]처럼 대괄호 라벨 한 줄로 쓰고, 섹션 사이는 빈 줄 하나로 구분하세요. 나열은 "- " 또는 "1." 을 쓰세요.`;

// spec 032 — 용어 보정 상시 통합: 기본 요약이 항상 치과 용어·치식 교정을 포함한다
// (별도 'dental' 엔진 선택 불필요 — 같은 AI 호출 1회라 원가 동일, 단가 2 유지).
const SUMMARY_PROMPT = (transcript: string) =>
  `다음은 치과 상담 녹취록입니다(음성 인식 결과라 치과 전문용어가 잘못 적혔을 수 있습니다).
크라운·인레이·온레이·크랙·신경치료·임플란트·스케일링·보철·교정 등 치과 용어와
치아 번호(예: #16, 6번 어금니)를 문맥에 맞게 바로잡으며, 핵심 내용을 자연스러운 상담 기록 형태로 요약해주세요.
[주소(Chief Complaint)] [진찰 소견] [처치 내용] [처방·권고 사항] 등 대괄호 라벨 섹션으로 간결하게 작성하되, 원문에 없는 내용은 추가하지 마세요.
출력은 반드시 다음 제목 줄로 시작하세요: "치과 상담 기록 요약 - Carelog"
녹취록 첫머리에 환자 이름이 언급되면, 제목 바로 다음 줄에 "환자: <이름>" 을 적으세요(이름이 없으면 그 줄은 생략).
${PLAINTEXT_RULES}
...
녹취록 첫머리에 환자 이름이 언급되면 제목 바로 다음 줄에 "환자: <이름>" 을 적으세요(없으면 생략).
${PLAINTEXT_RULES}
...
[요약]
치과 상담 기록 요약 - Carelog
(환자 증상·진찰 소견·처치·처방/권고를 [진찰 소견] 같은 대괄호 라벨 섹션으로 간결하게. 녹취록 첫머리에 환자 이름이 있으면 제목 다음 줄에 "환자: <이름>")
...
(굵게(**)·제목(#) 같은 마크다운 기호 금지 — 전자차트에 평문으로 붙습니다. 섹션이 필요하면 [진찰 소견]처럼 대괄호 라벨.)
